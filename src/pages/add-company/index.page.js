...
const AddCompany = () => {
    ...
    const uploadLogo = React.useCallback((setter, val) => {
      ...
      const onFinish = async () => {
        ...
        dispatch(addCompany({data: company, callback, navigateTo: () => router.push('/list-companies')}))
      };
    ...
    if(isAdded){
       return ( <Layout>
            ...
            <Row justify='center' align='center' className='auth-code mt-25'>
                <Col>
                    <Card
                        style={{ width: 300 }}
                        cover={<Image
                        ...
                        actions={[
                            <CopyOutlined onClick={() => {
                                if(typeof window != 'undefined'){
                                    navigator.clipboard.writeText(code.toLocaleUpperCase());
                    ...
                    >
                        <Meta
                        {...(logo ? {avatar: <Avatar src={logo} />} : {})}
                        {...(title ? {title: `Le code à partager pour l\'entreprise ${title} est le suivant:`} : {})}                        
                        {...(code ? {description: `${code.toLocaleUpperCase()}`} : {})}                        
                        />
                    </Card>
                </Col>
            </Row>
