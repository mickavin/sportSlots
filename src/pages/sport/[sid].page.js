...
import {editSport, getSport} from '../sports/actions'
...
const EditSport = () => {
    ...
    const sport = useSelector(s => s.sport.sportDetail.data)
    const router = useRouter();
    const routeForQueries = useRouteForQueries()
    const {sid} = routeForQueries.query

    useEffect(() => {
        if(sid){
            dispatch(getSport(sid))
        }
    }, [sid])
    ...
    useEffect(() => {
        if(sport){
            ...
            setLocalIcon(sport.localIcon)
            setIcon(sport.logo)
            setImage(sport.image)
            setBanner(sport.banner)
            setVideo(sport.video)
            formRef?.current?.setFieldsValue({
                title: sport.title,
                description: sport.description,
                icon: sport.localIcon
            })
        }
    }, [sport])
    ...
    const uploadLogo = React.useCallback((setter, val) => {
      ...
      const onFinish = async () => {
        ...
        dispatch(editSport({data: sport, navigateTo: () => router.push('/list-sports')}))
      };
