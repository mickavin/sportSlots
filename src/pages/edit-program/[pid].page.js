...
import {getCoachs, addProgram, getProgramForEdit, editProgram} from '../programs/actions'
...
const EditProgram = () => {
    ...
    const program = useSelector(s => s.program.programDetail.data)
    ...
    useEffect(() => {
        if(pid){
            dispatch(getProgramForEdit(pid))
        }
    }, [pid])
    ...
    useEffect(() => {
        if(program){
            ...
            setMail(program.mail)
            setImage(program.image)
            setSport(program.sport)
            setAssignableCoach(program.coach)
            setAssignableCompanies(program.companies)
            setStartDate(moment(program.startDate))
            setEndDate(moment(program.endDate))
            formRef?.current?.setFieldsValue({
                ...
                image: program.image,
                sport: program.sport,
                date: [moment(program.startDate), moment(program.endDate)]
            })
        }
    }, [program])
    ...
    const uploadLogo = React.useCallback((setter, val) => {
      ...
      const onFinish = async () => {
        ...
        dispatch(editProgram({data: program, callback, 
